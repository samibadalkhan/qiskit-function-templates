...
"""E2e tests for options — verifies executor options are accepted by the serverless
cluster and that invalid options cause the job to ERROR.

These tests run through the full serverless pipeline (not a fake backend), so they confirm
that options validation actually fires inside the running function — not just client-side.

Unlike the estimator-based vanilla template there is no resilience (ZNE/PEC) sub-model:
the executor caps mitigation at dynamical decoupling and twirling. Note also that enabling
twirling routes the program through the executor's samplex (boxed-layer) path, which only
runs on real hardware; the coverage test below leaves twirling disabled so it exercises the
plain-circuit path on any backend.
"""
