...
"""E2e tests for different PUB shapes submitted through serverless.

job.result() returns {"hw_results": PrimitiveResult, "metadata": {...}} —
all tests access result["hw_results"] for pub data.
"""
...
@ddt
class TestE2ePubs(BaseE2eTestCase):
    """Verify pub shapes and precision routing through the live serverless pipeline."""
    ...
    def test_min_parameters(self, num_pubs):
        ...
        job = self._func.run(backend_name=self._backend_name, pubs=pubs)
        result = job.result()
    ...
    def test_precision(self, pub_precision, default_precision):
        """Pub-level precision overrides default; default propagates when pub is unset."""
        circuit = random_circuit(num_qubits=2, depth=2, seed=42)
        observable = "Z" * circuit.num_qubits
        ...
        job = self._func.run(backend_name=self._backend_name, pubs=pubs, options=options)
        result = job.result()
